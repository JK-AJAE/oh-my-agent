...
type SourceFetcher = (args: FetchArgs) => Promise<SourceResult>;

function ok(source: string, items: SourceItem[]): SourceResult {
  return { source, items, failed: false };
}

function fail(source: string, reason: string): SourceResult {
