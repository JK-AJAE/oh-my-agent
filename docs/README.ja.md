...
| **oma-deepsec** | エージェント型脆弱性スキャナー (Vercel deepsec)、PR ゲート、カスタム matcher |
...
| 4 | `/deepsec` | エージェントベースの深層セキュリティスキャン (Vercel deepsec) |
...
**エージェント別モデル**: `.agents/oma-config.yaml` で各エージェントに独自のモデルと `effort` を割り当てられます。プリセットは6種類の runtime profile: `claude-only`、`codex-only`、`gemini-only`、`qwen-only`、`cursor-only`、`antigravity`。解決後の auth マトリクスは `oma doctor --profile` で確認できます。完全ガイド: [web/docs/guide/per-agent-models.md](../web/docs/guide/per-agent-models.md)。
