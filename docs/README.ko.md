...
| 에이전트 | 하는 일 |
|----------|------|
| **oma-academic-writer** | 출판 품질의 학술 영어 문장 작성·수정과 루브릭 기반 감사 |
...
| **oma-deepsec** | 에이전트 기반 취약점 스캐너, PR 게이트, 커스텀 매처 |
| **oma-design** | 디자인 시스템, 토큰, 접근성, 반응형 |
| **oma-dev-workflow** | CI/CD, 릴리스, 모노레포 자동화 |
| **oma-docs** | 참조 무결성 검사, 변경 영향 문서 식별 |
| **oma-frontend** | React/Next.js, TypeScript, Tailwind CSS v4, shadcn/ui |
| **oma-hwp** | HWP/HWPX/HWPML을 Markdown으로 변환 |
| **oma-image** | 멀티 벤더 AI 이미지 생성 |
| **oma-market** | 커뮤니티 시그널 기반 시장 조사, pain/trend/경쟁/discovery 의도와 SWOT/5F/PESTEL 자동 적용 |
| **oma-mobile** | Flutter 크로스플랫폼 앱 |
| **oma-observability** | APM/RUM, 메트릭/로그/트레이스/프로파일, SLO, 인시던트 포렌식, 전송 계층 튜닝을 다루는 관측성 라우터 |
...
| **oma-scholar** | 문헌 검색과 동료 평가를 함께하는 학술 연구 동반자 |
...
| **oma-voice** | Voicebox MCP 기반 로컬 우선 TTS/STT, 음성 생성·보이스오버·받아쓰기 |
...
**에이전트별 모델**: `.agents/oma-config.yaml`에서 각 에이전트마다 모델과 `effort`를 따로 지정할 수 있습니다. 6종의 runtime profile이 기본 제공됩니다: `claude`, `codex`, `gemini`, `qwen`, `cursor`, `mixed`. `oma doctor --profile`로 해석된 auth 매트릭스를 확인하세요. 전체 가이드: [web/docs/guide/per-agent-models.md](../web/docs/guide/per-agent-models.md).
