...
**按 agent 配置模型**：可在 `.agents/oma-config.yaml` 里为每个 agent 单独指定模型和 `effort`。内置六个 runtime profiles：`claude-only`、`codex-only`、`gemini-only`、`qwen-only`、`cursor-only`、`antigravity`。用 `oma doctor --profile` 查看解析后的 auth 矩阵。完整指南：[web/docs/guide/per-agent-models.md](../web/docs/guide/per-agent-models.md)。
