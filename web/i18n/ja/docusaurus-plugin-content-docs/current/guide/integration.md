...
description: 既存プロジェクトへのoh-my-agent追加ガイド。CLIパス、手動パス、検証、SSOTシンボリックリンク構造、インストーラーの内部動作を解説します。
---
...
Claude Codeシンボリックリンクは常に作成。インストーラーはAntigravity、Claude、Codex、Gemini、Qwen向けのベンダーネイティブのエージェントファイルとフックも生成します。GitHub Copilotは`.github/`があれば自動、なければ確認。
...
チェック項目：CLIインストール（agy、gemini、claude、codex、qwen）、認証、MCP設定、スキルステータス。
...
`installVendorAdaptations()`がサポートする全ベンダー（Antigravity、Claude、Codex、Gemini、Qwen）向けにIDE固有のファイルをインストールします。
